const app = new Vue({
	...
	created() {
		...
		const getCategories = () =>
			axios({ headers, method: "GET", url: `${site_url}categories` });
		const getCredits = () =>
			axios({ headers, method: "GET", url: `${site_url}credits` });
		const getDocuments = () =>
			axios({ headers, method: "GET", url: `${site_url}documents` });
		const getRecords = () =>
			axios({ headers, method: "GET", url: `${site_url}records` });
		const getStates = () =>
			axios({ headers, method: "GET", url: `${site_url}states` });
		const getPartners = () =>
			axios({ headers, method: "GET", url: `${site_url}partners` });
		const getMethods = () =>
			axios({ headers, method: "GET", url: `${site_url}methods` });
	...
	methods: {
		...
		calcValue(val) {
			return val == 1
				? this.credit.minAmount
				: (val * this.credit.maxAmount) / 100;
		},
		formatMoney(amount) {
		...
		filteredPartners() {
			const partners = this.partners
				.filter((partner) => {
					...
					const display =
						partner.onlyAgent == 0 ||
						(partner.onlyAgent == 1 && this.agent != null);
					...
					const moneyAvailable =
						partner.amounts.filter((a) => {
							const amount = this.calcValue(this.query.amount);
							return (
								amount >= parseFloat(a.from) && amount <= parseFloat(a.until)
							);
						}).length > 0;

					// check that the provider works with the selected state estado
					const stateAvailable =
						partner.states.filter((a) => a.id == this.query.state).length > 0;

					// Checks the credit is duable record
					const recordAvailable =
						partner.records.filter((a) => a.id == this.query.record).length > 0;

					// check if the document is valid document
					const documentAvailable =
						partner.documents.filter((a) => a.id == this.query.document)
							.length > 0;

					// check the category
					const categoryAvailable =
						partner.categories.filter((a) => a.id == this.query.category)
							.length > 0;

					// check the method
					const methodAvailable =
						partner.methods.filter((a) => a.id == this.query.payform).length >
						0;
